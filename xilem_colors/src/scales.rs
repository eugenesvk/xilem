use crate::{apca::estimate_lc, tokens::ThemeColor, color_space::{LinSrgb, Okhsl, from_degrees}};
use peniko::Color;
...
impl Scales {
    ...
    fn light_scale(&mut self) {
        ...
        for i in 0..12 {
            ...
            if (9..12).contains(&i) {
                self.okhsl[i] = Okhsl::from_color(rgb).darken(darken_values[i - 9]);
            }
            if i != 8 {
        ...
        let [x, y, z] = hsl.to_u8();
        let lc = estimate_lc(Color::WHITE, Color::rgb8(x, y, z));
        if lc > -46. {
    ...
    pub fn dark_scale(&mut self) {
        ...
        for i in 0..8 {
            self.rgbs[i] = rgb.darken(darken_values[i]);
            self.okhsl[i] = Okhsl::from_color(self.rgbs[i]);
        ...
        if (115.0..=220.).contains(&hue) {
            self.okhsl[11].saturation =
                self.okhsl[11].saturation.clamp(0.0, hsl.saturation * 0.75);
            self.okhsl[10].saturation =
                self.okhsl[10].saturation.clamp(0.0, hsl.saturation * 0.9);
        }
        let [x, y, z] = hsl.to_u8();
        let lc = estimate_lc(Color::WHITE, Color::rgb8(x, y, z));
        if lc < -95.4 {
